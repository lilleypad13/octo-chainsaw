...
public class EnemyBasicAI : MonoBehaviour {
    ...
    void Start()
    {
        ...
        target = go.transform;
        inFirstProjectile = false;
        inSecondProjectile = false;
    ...
    private void OnTriggerEnter2D(Collider2D coll)
    {
        if (inFirstProjectile == true && lastProjectileHitBy != coll.GetComponent<CircularSoundProjectileScript>().projectileId)
            //Will only run HitByNextProj if object is currently in the first projectile already, and the second projectile has a different 
            //projectileId value (different unique projectile)
        {
            HitByNextProj(coll);
        }
        HitByFirstProj(coll);
